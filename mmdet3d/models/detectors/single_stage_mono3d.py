...
from mmdet3d.models.builder import build_dtransform
...
class SingleStageMono3DDetector(SingleStageDetector):
    ...
    def __init__(self, backbone, neck=None, bbox_head=None, train_cfg=None, test_cfg=None, pretrained=None, init_cfg=None, depth_condition=None):
        super().__init__(backbone, neck, bbox_head, train_cfg, test_cfg, pretrained, init_cfg)
        self.depth_condition = None
        if depth_condition is not None:
            self.depth_condition = build_dtransform(depth_condition)

    def extract_feats(self, imgs, hindsight_points_depth_map=None):
        """Directly extract features from the backbone+neck."""
        assert isinstance(imgs, list)
        if self.depth_condition:
            feats = []
            for img, depth_map in zip(imgs, hindsight_points_depth_map):
                x = self.extract_feat(img)
                depth_feat = self.depth_condition(depth_map)
                x = [torch.cat((_x, _d), dim=1) for _x, _d in zip(x, depth_feat)]
                feats.append(x)
            return feats
        else:
    ...
    def forward_train(self,
        ...
        if self.depth_condition:
            ...
            depth_feat = self.depth_condition(hindsight_points_depth_map)
            x = [torch.cat((_x, _d), dim=1) for _x, _d in zip(x, depth_feat)]
    ...
    def simple_test(self, img, img_metas, hindsight_points_depth_map=None,
                    rescale=False):
        ...
        if self.depth_condition:
            assert hindsight_points_depth_map is not None
            depth_feat = self.depth_condition(hindsight_points_depth_map[0])
            x = [torch.cat((_x, _d), dim=1) for _x, _d in zip(x, depth_feat)]
    ...
    def aug_test(self, imgs, img_metas, hindsight_points_depth_map=None, rescale=False):
        """Test function with test time augmentation."""
        feats = self.extract_feats(imgs, hindsight_points_depth_map)
