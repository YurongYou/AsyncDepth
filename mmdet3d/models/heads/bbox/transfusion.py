...
from mmdet3d.utils import logger
...
class TransFusionHead(nn.Module):
    def __init__(
        ...
        if self.test_cfg["dataset"] == "nuScenes":
            warnings.warn("Using hard-coded dataset-specific information for transfusion")
            self.tasks = [
                dict(
                    num_class=8,
                    class_names=[],
            ...
            self.small_object_index = [8, 9]
        elif self.test_cfg["dataset"] == "Lyft":
            object_classes = [c.lower() for c in self.test_cfg["object_classes"]]
            small_object_index = []
            tasks = []

            # add the pedestrian task
            try:
                ind = object_classes.index('pedestrian')
                small_object_index.append(ind)
                tasks.append(dict(
                    num_class=1,
                    class_names=["pedestrian"],
                    indices=[ind],
            ...
            for ob in ['motorcycle', 'bicycle']:
                try:
                    ind = object_classes.index(ob)
                    small_object_index.append(ind)
                    task['num_class'] += 1
                    task['class_names'].append(ob)
                    task['indices'].append(ind)
                except ValueError:
                    pass
            ...
            remaining_class_index = list(set(range(len(object_classes))) - set(small_object_index))

            if remaining_class_index:
                tasks.append(dict(
                    num_class=len(remaining_class_index),
                    class_names=[],
                    indices=remaining_class_index,
                    radius=-1,
                ))
            

            self.small_object_index = small_object_index
            self.tasks = tasks[::-1] # not meaningful, just to make sure the order is the same as the original code
        elif self.test_cfg["dataset"] == "Ithaca365":
            object_classes = [c.lower() for c in self.test_cfg["object_classes"]]
            small_object_index = []
            tasks = []
            ...
            for ob in ['motorcyclist', 'bicyclist']:
                try:
                    ind = object_classes.index(ob)
                    small_object_index.append(ind)
                    task['num_class'] += 1
    ...
    def forward_single(self, inputs, img_inputs, metas):
        ...
        for i in self.small_object_index:
            local_max[:, i] = F.max_pool2d(heatmap[:, i], kernel_size=1, stride=1, padding=0)
        ...
        # elif self.test_cfg["dataset"] == "Lyft":  # for Pedestrian & Cyclist in Lyft
        #     local_max[
        #         :,
        #         5,
