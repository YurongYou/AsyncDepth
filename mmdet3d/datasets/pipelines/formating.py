...
class DefaultFormatBundle3D:
    ...
    def __call__(self, results):
        ...
        if "single_sweep_points" in results:
            assert isinstance(results["single_sweep_points"], BasePoints)
            results["single_sweep_points"] = DC(results["single_sweep_points"].tensor)
        ...
        if "img" in results:
            if isinstance(results['img'], list) and isinstance(results["img"][0], np.ndarray):
        ...
        if "hindsight_img" in results:
            # N_traversal, N_images, C, H, W
            results["hindsight_img"] = DC(torch.stack(
                [torch.stack(traversal) for traversal in results["hindsight_img"]]
            ), stack=True)

        if "hindsight_points" in results:
            # N_traversal, N_images, N_points, 4
            # results["hindsight_points"] = DC(torch.stack(
            #     [torch.stack(traversal) for traversal in results["hindsight_points"]]
            # ), stack=True)
            results["hindsight_points"] = DC([traversal.tensor for traversal in results["hindsight_points"]])

        if "hindsight_lidar2lidar" in results:
            # N_traversal, N_scan, 4, 4
            results["hindsight_lidar2lidar"] = DC([to_tensor(r) for r in results["hindsight_lidar2lidar"]])

        if "hindsight_points_depth_map" in results:
            results["hindsight_points_depth_map"] = DC([dmap for dmap in results["hindsight_points_depth_map"]])

        if "points_depth_map" in results:
            results["points_depth_map"] = DC(results["points_depth_map"])
