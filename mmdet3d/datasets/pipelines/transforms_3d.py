...
class AddLocNoise:
    def __init__(self, loc_noise_std=0.1):
        self.loc_noise_std = loc_noise_std

    def __call__(self, results):
        assert 'hindsight_points' in results
        error_vector = np.random.multivariate_normal(
            mean=(0,0,0),
            cov=np.eye(3), size=(1, ))
        error_vector /= np.linalg.norm(error_vector)
        error_vector *= np.random.normal(scale=self.loc_noise_std)
        for i in range(len(results["hindsight_points"])):
            results["hindsight_points"][i].translate(error_vector)
        return results

@PIPELINES.register_module()
class AddRotNoise:
    def __init__(self, rot_noise_std=0.1):
        self.rot_noise_std = rot_noise_std

    def __call__(self, results):
        assert 'hindsight_points' in results
        random_rot_z = np.random.normal(scale=self.rot_noise_std / 180 * np.pi)
        for i in range(len(results["hindsight_points"])):
            results["hindsight_points"][i].rotate(random_rot_z)
        return results

@PIPELINES.register_module()
class ImageAug3D:
    def __init__(
        self, final_dim, resize_lim, bot_pct_lim, rot_lim, rand_flip, is_train, rotate_first=False, on_hindsight_images=False
    ):
        ...
        self.on_hindsight_images = on_hindsight_images
    ...
    def img_transform(
        self, img, rotation, translation, resize, resize_dims, crop, flip, rotate, ori_shape, rotate_first=False
    ):
        if rotate_first:
            # adjust image
            ...
            # post-homography transformation
            rotation *= resize
            # Yurong: here the resizing on the translation is skipped since it is zero initially
            translation -= torch.Tensor(crop[:2])
            if flip:
                A = torch.Tensor([[-1, 0], [0, 1]])
                b = torch.Tensor([crop[2] - crop[0], 0]) # YY: should be crop[2] - crop[0] - 1?
                rotation = A.matmul(rotation)
                translation = A.matmul(translation) + b
            theta = rotate / 180 * np.pi
            A = torch.Tensor(
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.on_hindsight_images:
            imgs = data["img"]
            new_imgs = []
            transforms = []
            for img in imgs:
                new_img, transform = process_img(img)
            ...
            hindsight_img = data["hindsight_img"]
            new_imgs = []
            transforms = []
            for traversal in hindsight_img:
                new_imgs.append([])
                transforms.append([])
                for img in traversal:
                    new_img, transform = process_img(img)
                    new_imgs[-1].append(new_img)
                    transforms[-1].append(transform.numpy())
            data["hindsight_img"] = new_imgs
            data["hindsight_img_aug_matrix"] = transforms
        return data
...
class GlobalRotScaleTrans:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.is_train:
            ...
            if "single_sweep_points" in data:
                data["single_sweep_points"].rotate(-theta)
                data["single_sweep_points"].translate(translation)
                data["single_sweep_points"].scale(scale)

            if "hindsight_points" in data:
                for i in range(len(data["hindsight_points"])):
                    data["hindsight_points"][i].rotate(-theta)
                    data["hindsight_points"][i].translate(translation)
                    data["hindsight_points"][i].scale(scale)
...
class RandomFlip3D:
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if flip_horizontal:
            ...
            if "single_sweep_points" in data:
                data["single_sweep_points"].flip("horizontal")

            if "hindsight_points" in data:
                for i in range(len(data["hindsight_points"])):
                    data["hindsight_points"][i].flip("horizontal")
        ...
        if flip_vertical:
            ...
            if "single_sweep_points" in data:
                data["single_sweep_points"].flip("vertical")

            if "hindsight_points" in data:
                for i in range(len(data["hindsight_points"])):
                    data["hindsight_points"][i].flip("vertical")
...
@PIPELINES.register_module()
class RandomFlip3DOriginal(RandomFlip):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(RandomFlip3DOriginal, self).__init__(
    ...
    def __call__(self, input_dict):
        ...
        super(RandomFlip3DOriginal, self).__call__(input_dict)
...
class PointShuffle:
    def __call__(self, data):
        ...
        if "hindsight_points" in data:
            for i in range(len(data["hindsight_points"])):
                data["hindsight_points"][i].shuffle()
        return data
...
class PointsRangeFilter:
    ...
    def __call__(self, data):
        ...
        if "hindsight_points" in data:
            for i in range(len(data["hindsight_points"])):
                points = data["hindsight_points"][i]
                points_mask = points.in_range_3d(self.pcd_range)
                clean_points = points[points_mask]
                data["hindsight_points"][i] = clean_points
        return data
...
class ImageNormalize:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "hindsight_img" in data:
            data["hindsight_img"] = [
                [self.compose(img) for img in traversal]
                    for traversal in data["hindsight_img"]
            ]
            data["hindsight_img_norm_cfg"] = dict(mean=self.mean, std=self.std)
        return data
...
class ImageDistort:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for img in imgs:
            if self.from_to_pil:
                img = np.array(img).astype(np.float32)[:, :, ::-1]
...
class GenerateDepthMap:
    '''
        Generate depth map based on point cloud
        This function will assume that the point cloud and images has gone through some transformation

        Args:
            reduction_method (str): method to reduce overlapping depth values to a single depth value
            filler (float): value to fill in the depth map where there is no point cloud
    '''
    def __init__(
        self,
        ...
        Minkowski_sparsify_resolution: float = -1.0,
        normalize_depth: float = -1.0
    ):
        assert reduction_method in ['min', 'mean', 'random', 'max']
        self.reduction_method = reduction_method
        self.filler = filler
        self.channel_first = channel_first
        self.Minkowski_sparsify_resolution = Minkowski_sparsify_resolution
        self.normalize_depth = normalize_depth


    def _construct_depth_map(self, lidar, lidar_aug_matrix, img_aug_matrix, lidar2image, image_resolution):
        h, w = image_resolution
         # Np x 3
        lidar -= lidar_aug_matrix[:3, 3]
        # 3 x Np
        lidar = torch.inverse(lidar_aug_matrix[:3, :3]).matmul(lidar.transpose(1, 0))

        # project to the image
        # Nc x Np x 3
        coor_image = lidar2image[:, :3, :3].matmul(lidar).transpose(-1, -2)
        coor_image += lidar2image[:, :3, 3].reshape(-1, 1, 3)

        raw_depth_map = copy.deepcopy(coor_image[:, :, -1])


        # negative depth points will be mapped to so close to the camera that they are not visible
        coor_image[..., -1] = torch.clamp(coor_image[..., -1], 1e-5, 1e5)
        coor_image /= coor_image[..., [-1]]

        coor_image = torch.bmm(coor_image, img_aug_matrix[:, :3, :3].transpose(-1, -2))
        coor_image += img_aug_matrix[:, :3, 3].reshape(-1, 1, 3)
        ...
        depth_map = torch.zeros((on_img.shape[0], h, w))

        for c in range(on_img.shape[0]):
            masked_coords = coor_image[c, on_img[c]].long()
            masked_dist = raw_depth_map[c, on_img[c]]

            if self.reduction_method == 'min':
                linear_ind = masked_coords[..., 0] + masked_coords[..., 1] * w
                reduced_depth = torch.full((h*w, ), fill_value=float("Inf")).float()
                reduced_depth, _ = scatter_min(masked_dist, index=linear_ind, out=reduced_depth)
                reduced_depth = reduced_depth.reshape(h, w)
                reduced_depth[torch.isinf(reduced_depth)] = self.filler
            elif self.reduction_method == 'max':
                linear_ind = masked_coords[..., 0] + masked_coords[..., 1] * w
                reduced_depth = torch.full((h*w, ), fill_value=-1).float()
                reduced_depth, _ = scatter_max(masked_dist, index=linear_ind, out=reduced_depth)
                reduced_depth = reduced_depth.reshape(h, w)
                reduced_depth[reduced_depth < 0] = self.filler
            elif self.reduction_method == 'mean':
                linear_ind = masked_coords[..., 0] + masked_coords[..., 1] * w
                reduced_depth = torch.full((h*w, ), fill_value=0.0).float()
                reduced_depth = scatter_mean(masked_dist, index=linear_ind, out=reduced_depth)
                reduced_depth = reduced_depth.reshape(h, w)
                reduced_depth[torch.isclose(reduced_depth, torch.zeros(1))] = self.filler
            elif self.reduction_method == 'random':
                reduced_depth = torch.zeros((h, w))
                reduced_depth[masked_coords[:, 1], masked_coords[:, 0]] = masked_dist
                reduced_depth[torch.isclose(reduced_depth, torch.zeros(1))] = self.filler

            if self.normalize_depth > 0:
                reduced_depth[reduced_depth > 0] = reduced_depth[reduced_depth > 0] / self.normalize_depth
            depth_map[c] = reduced_depth

        return depth_map

    def __voxelize_centroid(self, points):
        '''
            Voxelize using Minkowski Engine and represent each voxel with its centroid
        '''
        return ME.utils.sparse_quantize(points / self.Minkowski_sparsify_resolution).float() * self.Minkowski_sparsify_resolution + self.Minkowski_sparsify_resolution / 2



    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        if self.channel_first:
            _, h, w = data['img'][0].shape
        else:
            h, w, _ = data['img'][0].shape
        if 'points' in data:
            # Nx3
            lidar = copy.deepcopy(data['points'].coord)

            if self.Minkowski_sparsify_resolution > 0:
                lidar = self.__voxelize_centroid(lidar)

            lidar_aug_matrix = torch.from_numpy(data['lidar_aug_matrix'])
            img_aug_matrix = torch.from_numpy(np.stack(data['img_aug_matrix']))
            lidar2image = torch.from_numpy(np.stack(data['lidar2image']))

            depth_map = self._construct_depth_map(lidar, lidar_aug_matrix, img_aug_matrix, lidar2image, (h, w))

            data['points_depth_map'] = depth_map
        ...
        if 'hindsight_points' in data:
            ...
            if 'img_aug_matrix' in data:
                ...
                img_aug_matrix = torch.eye(4, dtype=torch.float32)[None, ...].repeat(len(data['img']), 1, 1)
            if 'lidar2image' in data:
                lidar2image = torch.from_numpy(np.stack(data['lidar2image']))
            elif 'lidar2img' in data:
                lidar2image = torch.from_numpy(np.stack(data['lidar2img'])).float()
            else:
                raise NotImplementedError()

            hindsight_depth_map = []
            for p in data['hindsight_points']:
                lidar = copy.deepcopy(p.coord)
                if self.Minkowski_sparsify_resolution > 0:
                    lidar = self.__voxelize_centroid(lidar)
                hindsight_depth_map.append(self._construct_depth_map(lidar, lidar_aug_matrix, img_aug_matrix, lidar2image, (h, w)))

            data['hindsight_points_depth_map'] = hindsight_depth_map
        return data


@PIPELINES.register_module()
class GenerateDepthMapSingle:
    '''
        Generate depth map based on point cloud
        This function will assume that the point cloud and images has gone through some transformation
    ...
    def _construct_depth_map(self, lidar, lidar_aug_matrix, img_aug_matrix, lidar2image, image_resolution):
        ...
        # Np x 3
        coor_image = lidar2image[:3, :3].matmul(lidar).transpose(-1, -2)
        coor_image += lidar2image[:3, 3].reshape(1, 3)
        ...
        coor_image = torch.mm(coor_image, img_aug_matrix[:3, :3].transpose(-1, -2))
        coor_image += img_aug_matrix[:3, 3].reshape(1, 3)
        ...
        masked_coords = coor_image[on_img].long()
        masked_dist = raw_depth_map[on_img]
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'points' in data and self.use_gt:
            # Nx3
            lidar = copy.deepcopy(data['points'].coord)
            ...
            if 'img_aug_matrix' in data:
                ...
                img_aug_matrix = torch.eye(4, dtype=torch.float32)
                if data.get("pcd_horizontal_flip", False):
                    img_aug_matrix[:2, :2] = torch.Tensor([[-1, 0], [0, 1]])
                    origin_w = data['ori_shape'][1] if not self.channel_first else data['ori_shape'][2]
                    img_aug_matrix[:2, 3] = torch.Tensor([origin_w - 1, 0])
            if 'lidar2image' in data:
                lidar2image = torch.from_numpy(data['lidar2image'])
            elif 'lidar2img' in data:
                lidar2image = torch.from_numpy(data['lidar2img']).float()
            else:
                raise NotImplementedError()

            depth_map = self._construct_depth_map(lidar, lidar_aug_matrix, img_aug_matrix, lidar2image, (h, w))

            data['hindsight_points_depth_map'] = [depth_map]

        if 'hindsight_points' in data and not self.use_gt:
            if 'lidar_aug_matrix' in data:
                lidar_aug_matrix = torch.from_numpy(data['lidar_aug_matrix'])
            else:
                lidar_aug_matrix = torch.eye(4, dtype=torch.float32)
