...
from .ithaca365_dataset import format_ithaca365_results, ithaca365DetectionConfig
...
class Ithaca365MonoDataset(CocoDataset):
    r"""Monocular 3D detection on Ithaca365 Dataset.
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            boxes = cam_ithaca365_box_to_global(self.data_infos[sample_id],
                                                  boxes)
            ...
            if (sample_id + 1) % CAM_NUM != 0:
                continue
            boxes = global_ithaca365_box_to_cam(
                self.data_infos[sample_id + 1 - CAM_NUM], boxes_per_frame)
            cam_boxes3d, scores, labels = ithaca365_box_to_cam_box3d(boxes)
            ...
            boxes = output_to_ithaca365_box(det)
            boxes = cam_ithaca365_box_to_global(
            ...
            for i, box in enumerate(boxes):
                ...
                annos.append(ithaca365_anno)
            # other views results of the same frame should be concatenated
            if sample_token in ithaca365_annos:
                ithaca365_annos[sample_token].extend(annos)
            else:
                ithaca365_annos[sample_token] = annos

        nusc_submissions = {
            'meta': self.modality,
            'results': ithaca365_annos,
        }
...
def cam_ithaca365_box_to_global(info, boxes):
    """Convert the box from camera to global coordinate.
...
def global_ithaca365_box_to_cam(info,
                           boxes,):
    """Convert the box from global to camera coordinate.
...
def ithaca365_box_to_cam_box3d(boxes):
    """Convert boxes from :obj:`NuScenesBox` to :obj:`CameraInstance3DBoxes`.
