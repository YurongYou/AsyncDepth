...
class LyftMonoDataset(CocoDataset):
    r"""Monocular 3D detection on Lyft Dataset.
    ...
    def __init__(self,
        ...
        if self.modality.get('use_hindsight_lidar', False):
            assert hindsight_info_file is not None
            hindsight_info = mmcv.load(hindsight_info_file)
            self.tkn2hindsight_info = {info['token']: info for info in hindsight_info['infos']}
    ...
    def _load_hindsight(self, img_info, results):
        hindsight_info = self.tkn2hindsight_info[img_info['token']]

        # lidar to camera transform
        lidar2camera_r = np.linalg.inv(img_info["sensor2lidar_rotation"])
        lidar2camera_t = (
            img_info["sensor2lidar_translation"] @ lidar2camera_r.T
        )
        ...
        camera_intrinsics[:3, :3] = img_info["camera_intrinsics"]
        lidar2image = camera_intrinsics @ lidar2camera_rt.T

        results["lidar2image"] = lidar2image
        results["hindsight_lidar_paths"] = []
        results["hindsight_lidar2lidar"] = []

        for traversal in hindsight_info["history_traversals"]:
            results["hindsight_lidar_paths"].append([])
            results["hindsight_lidar2lidar"].append([])
            ...
            for lidar_info in traversal_sweeps:
                ...
                results["hindsight_lidar_paths"][-1].append(data_path)
                ...
                results["hindsight_lidar2lidar"][-1].append(sensor2lidar)

    def _load_lidar(self, img_info, results):
        info = self.tkn2hindsight_info[img_info['token']]
        ...
        if self.load_relative_path:
            data['lidar_path'] = osp.join(self.data_root, data['lidar_path'])
    ...
    def prepare_train_img(self, idx):
        ...
        if self.load_relative_path:
            img_info['filename'] = osp.join(self.data_root, img_info['filename'])
        ...
        if self.modality.get('use_lidar', False):
            self._load_lidar(img_info, results)

        if self.modality.get('use_hindsight_lidar', False):
            self._load_hindsight(img_info, results)
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            boxes = cam_lyft_box_to_global(self.data_infos[sample_id],
                                                  boxes)
            ...
            if (sample_id + 1) % CAM_NUM != 0:
                continue
            boxes = global_lyft_box_to_cam(
                self.data_infos[sample_id + 1 - CAM_NUM], boxes_per_frame)
            cam_boxes3d, scores, labels = lyft_box_to_cam_box3d(boxes)
            ...
            cam_boxes3d = CameraInstance3DBoxes(boxes3d, box_dim=7)
            det = bbox3d2result(cam_boxes3d, scores, labels)
            boxes = output_to_lyft_box(det)
            boxes = cam_lyft_box_to_global(
            ...
            if sample_token in lyft_annos:
                lyft_annos[sample_token].extend(annos)
            else:
    ...
    def _evaluate_single(self,
        ...
        # }
        valid_sample_tokens = set([x['token'] for x in self.data_infos])
        metric_prefix = f'{result_name}_Lyft'

        if eval_by_distance:
            metrics, detail = lyft_eval_by_distance(
                lyft, valid_sample_tokens, result_path, output_dir, logger, metric_prefix, close_only=close_only, **kwargs)
        else:
...
def cam_lyft_box_to_global(info, boxes):
    """Convert the box from camera to global coordinate.
    ...
    for box in boxes:
        # Move box to ego vehicle coord system
        box.rotate_around_origin(pyquaternion.Quaternion(info['cam2ego_rotation']))
        box.translate(np.array(info['cam2ego_translation']))
        # Move box to global coord system
...
def global_lyft_box_to_cam(info,
                           boxes,):
    """Convert the box from global to camera coordinate.
    ...
    for box in boxes:
        ...
        # Move box to camera coord system
        box.translate(-np.array(info['cam2ego_translation']))
        box.rotate_around_origin(pyquaternion.Quaternion(info['cam2ego_rotation']).inverse)
        box_list.append(box)
    return box_list
...
def lyft_box_to_cam_box3d(boxes):
    """Convert boxes from :obj:`NuScenesBox` to :obj:`CameraInstance3DBoxes`.
    ...
    boxes_3d = torch.cat([locs, dims, rots], dim=1).cuda()
    cam_boxes3d = CameraInstance3DBoxes(
        boxes_3d, box_dim=7, origin=(0.5, 0.5, 0.5))
    scores = torch.Tensor([b.score for b in boxes]).cuda()
    labels = torch.LongTensor([b.label for b in boxes]).cuda()
    nms_scores = scores.new_zeros(scores.shape[0], 10 + 1)
    indices = labels.new_tensor(list(range(scores.shape[0])))
