...
from ithaca365.eval.detection.data_classes import DetectionConfig as ithaca365DetectionConfig
from ithaca365.eval.detection.data_classes import DetectionBox as ithaca365DetectionBox
class nuscDetectionConfig(ithaca365DetectionConfig):
...
class nuscDetectionBox(ithaca365DetectionBox):
    def __init__(self,
                 sample_token: str = "",
...
class NuScenesDataset(Custom3DDataset):
    ...
    def _evaluate_single(
        ...
        **kwargs
        ...
        if eval_by_distance:
            # metrics, detail = nusc_eval_by_distance(
            #     nusc, valid_sample_tokens,
            #     result_path, output_dir, logger,
            #     metric_prefix)
            from nuscenes.eval.detection.data_classes import DetectionBox
            from ithaca365.eval.detection.evaluate import DetectionEval as DistDetectionEval
            from nuscenes.eval.common.loaders import load_gt
            eval_config = nuscDetectionConfig(
                class_range = {
                    "car": [[0, 30], [30, 50], [50, 80], [0, 80]],
                    "truck": [[0, 30], [30, 50], [50, 80], [0, 80]],
                    "bus": [[0, 30], [30, 50], [50, 80], [0, 80]],
                    "trailer": [[0, 30], [30, 50], [50, 80], [0, 80]],
                    "construction_vehicle": [[0, 30], [30, 50], [50, 80], [0, 80]],
                    "pedestrian": [[0, 30], [30, 50], [50, 80], [0, 80]],
                    "motorcycle": [[0, 30], [30, 50], [50, 80], [0, 80]],
                    "bicycle": [[0, 30], [30, 50], [50, 80], [0, 80]],
                    "traffic_cone": [[0, 30], [30, 50], [50, 80], [0, 80]],
                    "barrier": [[0, 30], [30, 50], [50, 80], [0, 80]]
                },
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if "boxes_3d" in results[0] or "pts_bbox" in results[0]:
            result_files, tmp_dir = self.format_results(results, jsonfile_prefix)

            if isinstance(result_files, dict):
                for name in result_names:
                    print("Evaluating bboxes of {}".format(name))
                    ret_dict = self._evaluate_single(result_files[name], **kwargs)
                metrics.update(ret_dict)
            elif isinstance(result_files, str):
                metrics.update(self._evaluate_single(result_files, **kwargs))
...
def output_to_nusc_box(detection):
    ...
    """
    if "boxes_3d" in detection:
        box3d = detection["boxes_3d"]
        scores = detection["scores_3d"].numpy()
        labels = detection["labels_3d"].numpy()
    else:
        box3d = detection["pts_bbox"]["boxes_3d"]
        scores = detection["pts_bbox"]["scores_3d"].numpy()
        labels = detection["pts_bbox"]["labels_3d"].numpy()
