...
from mmdet3d.datasets import Ithaca365Dataset
...
def get_2d_boxes(nusc, sample_data_token: str, visibilities: List[str], mono3d=True, dataset_type='nusc'):
    ...
    for ann_rec in ann_recs:
        ...
        if isinstance(box, LyftBox):
            box.rotate_around_origin(Quaternion(pose_rec["rotation"]).inverse)
        else:
        ...
        if isinstance(box, LyftBox):
            box.rotate_around_origin(Quaternion(cs_rec["rotation"]).inverse)
        else:
        ...
        repro_rec = generate_record(
            ann_rec, min_x, min_y, max_x, max_y, sample_data_token, sd_rec["filename"], dataset_type
        )
        ...
        if mono3d and (repro_rec is not None):
            ...
            if dataset_type in ['lyft', 'ithaca365']:
                repro_rec["attribute_name"] = ""
...
def generate_record(
    ...
    if dataset_type == 'lyft':
        if repro_rec["category_name"] not in LyftDataset.NameMapping:
            return None
        cat_name = LyftDataset.NameMapping[repro_rec["category_name"]]
        coco_rec["category_id"] = lyft_categories.index(cat_name)
    elif dataset_type == 'ithaca365':
        if repro_rec["category_name"] not in Ithaca365Dataset.NameMapping:
            return None
        cat_name = Ithaca365Dataset.NameMapping[repro_rec["category_name"]]
        coco_rec["category_id"] = ithaca365_categories.index(cat_name)
    else:
